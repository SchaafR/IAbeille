  ...
  while ((millis() - t0) < 3000) {
    if (recv_packet(type, payload, len, 800000)) {
      if (type == RESULT && len >= 8) {
        ...
        uint16_t hornet = (uint16_t)payload[1] | ((uint16_t)payload[2] << 8);
        uint16_t bee = (uint16_t)payload[3] | ((uint16_t)payload[4] << 8);
        uint16_t bees = (uint16_t)payload[5] | ((uint16_t)payload[6] << 8);

        uint8_t status = payload[7];
