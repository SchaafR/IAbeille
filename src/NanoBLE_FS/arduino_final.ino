  ...
  switch (etatActuel) {
    ...
    case ETAT_SOMMEIL:
      ...
      if (modeFonctionnement == 2) {
        // mode test : 20 secondes
        entrerModeSommeil(20000);
      } else {
        // modes normaux : 20 mins
        entrerModeSommeil(1200000);
      }
    ...
    case ETAT_VERIFICATION:
      ...
      if (lastTemp > 12.0 && lastLux > 100.0) {
        ...
        if (modeFonctionnement == 2) {
          ...
          if (cyclesSansEnvoi >= 5) {
            etatActuel = ETAT_ENVOI_LORAWAN;
          }
          else if(comptForceCam >= 2){
            DEBUG_PRINTLN("-> Precision : camera ");
            etatActuel = ETAT_ANALYSE_CAMERA;
          }
          else {
            DEBUG_PRINTLN("-> Precision : audio ");
        ...
        cyclesSansEnvoi++;
        if (cyclesSansEnvoi >= 5) {
          lastHornet_sound = 0;  // Sera utilisé pour t_5
          lastQueen_sound = 0;   // Sera utilisé pour t_4
          lastHornet = 0;        // Sera utilisé pour t_1
          lastBee = 0;           // Sera utilisé pour t_2
          lastBees = 0;          // Sera utilisé pour t_3
          lastStatus = 0;        // Sera utilisé pour t_0 (0-3)
          etatActuel = ETAT_ENVOI_LORAWAN;
        }
      }
    ...
    case ETAT_ANALYSE_AUDIO:
      {
        ...
        lastHornet_sound = (uint8_t)(probaHornet * 100);

        if (probaHornet > 0.70) {
          DEBUG_PRINTLN("Camera");
          etatActuel = ETAT_ANALYSE_CAMERA;
        } else {
          //DEBUG_PRINTLN("=> Rien de suspect. Retour vérification.");
          etatActuel = ETAT_SOMMEIL;
        }
      }
      break;
